...
var adarr = home_admin.get_adlist();
		...
		var handler = function(req,res,result){
		  ...
		  res.render(renderPage,{"session":req.session,"adarr":adarr,data:result,params:req.params,idx:idxv});
       }
...
var adarr = home_admin.get_adlist();
	renderPage  = "timer";
	    if (req.session.logined=="yes") {
                ...
                res.send(`<script>alert(\"로그인 하셔야 이용가능합니다.\");</script><script>location.href=\"https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=${naver_credential.client_id}&redirect_uri=${naver_credential.callback_uri}&state=66467&svctype=0\";</script>`);
        }
});
...
var adarr = home_admin.get_adlist();
	...
	if(username==""||username==undefined||userphone==""||userphone==undefined){
	  res.send("<script>alert(\"이름과 휴대폰번호를 입력해주세요.\");</script><script>location.href=\"/booking_reservation\";</script>");   
	}else{
	res.render(renderPage,{"session":req.session,"adarr":adarr,"username":username,"userphone":userphone});
	}
});
...
var handler = function(req,res,result){
          ...
          if(req.body.room_twoprice!="0"){
            ...
            res.render(renderPage,{f_day:req.body.f_day, l_day:req.body.l_day, price:req.body.room_price,twoprice:req.body.room_twoprice, days:req.body.days, sec_day:req.body.sec_day, "session":req.session,"adarr":adarr,data:result,params:req.params,idx:idxv});   
          }else{
           res.render(renderPage,{f_day:req.body.f_day, l_day:req.body.l_day, price:req.body.room_price, days:req.body.days, sec_day:req.body.sec_day, "session":req.session,"adarr":adarr,data:result,params:req.params,idx:idxv});
          }
       }
