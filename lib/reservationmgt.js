...
module.exports = (function () {
  ...
  reservationmgt.reservation_refund = function (req, res, handler) {
    ...
    connection.query(sql, req.body.args, function (err, rows, fields) {
      if (err) {
        ...
        //예약 승인와 환불 신청 상태일 경우에만 실행
        if (rows[0].rev_state == '2' || rows[0].rev_state == '3') { //2: 예약 승인, 3: 환불 신청
          var data;
   ...
   * 환불 신청 (사용자 용)
   */
    reservationmgt.reservation_refund_with_info = function (req, res) {
      if (!req.body.args || !req.body.name || !req.body.bank || !req.body.account_number) {
        res.send(
          '<script>alert("환불 신청 정보를 전부 입력해주세요.")</script> <script>history.back()</script>',
        );
        return;
      }

      reservationmgt.reservation_refund(req, res, function(req) {
        register_refund_info({
          rev_id: req.body.args,
          name: req.body.name,
          bank: req.body.bank,
  ...
  /**
   * 환불 신청 정보 등록 
   * @param {{ rev_id: string, name: string, bank: string, account_number: string }}
   * rev_id 예약 정보 key (ex: first_etc&2022-10-10)
   * name 예금주
   * bank 은행
   * account_number 계좌 정보
   */
  function register_refund_info({ rev_id, name, bank, account_number }) {
    var insertSql = 'INSERT INTO refund_info (rev_id, name, bank, account_number) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE name=VALUES(name), bank=VALUES(bank), account_number=VALUES(account_number)';
  ...
  reservationmgt.get_reservation_refund_info = function (req, res) {
    const sql = "select rev_id, name, bank, account_number, refund_date from refund_info where rev_id=?";
    const rev_id = req.query.rev_id;

    if (!rev_id) {
      error_handler(res);
    } else {
      connection.query(sql, rev_id, function (err, rows, fields) {
        if (err) {
          console.log(err);
          error_handler(res);
        } else {
    ...
    function success_handler(res, row) {
      res.json({
        status: "success",
        result: {
          rev_id: row.rev_id,
