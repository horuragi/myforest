...
module.exports = (function() {
      ...
      reservationmgt.reservation_confirm = function(req, res) {
        ...
        var data;
        var sql = "select *, date_format(rev_lday, '%Y-%m-%d') AS l_day from (select *,SUBSTRING_INDEX(room_reservation.rev_id, '&', -1) rev_id_split, date_format(rev_sday, '%Y-%m-%d') AS s_day from room_reservation)day where rev_name=?";
      ...
      reservationmgt.reservation_admin_query = function(req, res) {
        ...
        var sql = "select *, SUBSTRING_INDEX(rev_id, '&', -1) AS rev_id_split, date_format(rev_sday, '%Y-%m-%d') AS s_day, date_format(rev_lday, '%Y-%m-%d') AS l_day from room_reservation";
      ...
      reservationmgt.reservation_admin_state_query = function(req, res) {
        ...
        var sql = "select *, SUBSTRING_INDEX(rev_id, '&', -1) AS rev_id_split, date_format(rev_sday, '%Y-%m-%d') AS s_day, date_format(rev_lday, '%Y-%m-%d') AS l_day from room_reservation where rev_state=? and rev_name like ?";
        const rev_state = req.body.args || "0";
        const rev_name = req.body.rev_name ? `%${req.body.rev_name}%` : '%%';
        let args = [rev_state, rev_name];
        
        if(rev_state === "0") {
          sql = "select *, SUBSTRING_INDEX(rev_id, '&', -1) AS rev_id_split, date_format(rev_sday, '%Y-%m-%d') AS s_day, date_format(rev_lday, '%Y-%m-%d') AS l_day from room_reservation where rev_name like ?"
          args = [rev_name];
        }
      ...
      reservationmgt.reservation_admin_refund_query = function(req, res) {
        ...
        var sql = "select *, date_format(rev_lday, '%Y-%m-%d') AS l_day from (select *,SUBSTRING_INDEX(last_reservation.rev_id, '&', -1) rev_id_split, date_format(rev_sday, '%Y-%m-%d') AS s_day from last_reservation)day where rev_state=? and rev_name like ?";
      ...
      reservationmgt.reservation_cancel = function(req, res) {
        ...
        var sql = "select *, date_format(DATE_sub(rev_lday, INTERVAL 1 DAY), '%Y-%m-%d') AS l_day from (select *, date_format(rev_sday, '%Y-%m-%d') AS s_day from room_reservation)day where rev_id=?";
            ...
            if (rows[0].rev_state == "1") {
              ...
              connection.query(sql, data, function(err, rows, fields) {
                if (err) {
                  ...
                  res.send("<script>alert(\"예약 취소 완료\")</script> <script>location.href=\"/booking_reservation\"</script>");
                }
              });
            } else {
              res.send("<script>alert(\"예약 신청 상태일 경우에만 취소가 가능 합니다.\")</script> <script>history.back()</script>");
            }
          }
        });
      }
            ...
            if (rows[0].rev_state == "2") {
              ...
              connection.query(sql, data, function(err, rows, fields) {
                if (err) {
                  ...
                  res.send("<script>alert(\"환불 신청 완료\")</script> <script>location.href=\"/booking_reservation\"</script>");
                }
              });
            } else {
              res.send("<script>alert(\"예약 승인 상태일 경우에만 환불 신청이 가능 합니다.\")</script> <script>history.back()</script>");
            }
          }
        });
      }
      ...
      reservationmgt.reservation_refund_auth = function(req, res) {
        ...
        connection.query(sql, req.body.args, function(err, rows, fields) {
            if (err) {
              ...
              if (rows[0].rev_state == "3") {
                ...
                var data;
                var sql = "INSERT INTO last_reservation(rev_id, rev_roomtype, room_name, user_id, rev_name, rev_phone, rev_sday, rev_lday, rev_charge, rev_cancel, rev_state) VALUES(?,?,?,?,?,?,?,?,?,?,?)";

                data = [rows[0].rev_id, rows[0].rev_roomtype, rows[0].room_name, rows[0].user_id, rows[0].rev_name, rows[0].rev_phone, rows[0].rev_sday, rows[0].rev_lday, rows[0].rev_charge, rows[0].rev_cancel, 5]
        ...
        reservationmgt.room = function(req, res) {
          ...
          var sql = 'select room_management.room_type,room_management.room_name,room_detail.room_lowcharge,room_detail.room_peakcharge,room_management.room_type type_args,room_management.room_name name_args from room_management, room_detail where room_management.room_type=room_detail.room_type;';
