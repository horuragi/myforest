...
reserve.room_inquiry=function(req,res){
  ...
  var sql = 'select room_type, count(*) AS roomcount from (select distinct room_type, room_name from (select * from rev_info where rev_day >= date(?) and rev_day < date(?))datequery )duplication group by room_type';
  ...
  if (parseInt(req.body.f_day[1])==6 && parseInt(req.body.l_day[1])==7) {
    ...
    overday=parseInt(req.body.l_day[2])-1;
    status=4;
  } else if ((parseInt(req.body.f_day[1])==7 && parseInt(req.body.l_day[1])==7) || (parseInt(req.body.f_day[1])==7 && parseInt(req.body.l_day[1])==8) || (parseInt(req.body.f_day[1])==8 && parseInt(req.body.l_day[1])==8)) {
    status=2;
  } else {
    status=1;
  }

  mysql.room_info(res, "select * from room_detail",errhandler, function(result) {
  ...
  mysql.query_get(res, "select * from room_detail", errhandler, function(result) {
    if (status==1) {
      ...
      if ( day-overday >= 2) {
        ...
        room_result.scops_owl_price = (result[7].room_lowcharge * (day-overday)) + (result[7].room_peakcharge * overday);
        room_result.site_cypress_price = (result[9].room_lowcharge * (day-overday)) + (result[9].room_peakcharge * overday);
        room_result.site_zelkova_price = (result[11].room_lowcharge * (day-overday)) + (result[11].room_peakcharge * overday);
        room_result.site_grass_price = (result[10].room_lowcharge * (day-overday)) + (result[10].room_peakcharge * overday);
        room_result.firstetc_price = (result[2].room_lowcharge * (day-overday)) + (result[2].room_peakcharge * overday);
        room_result.secondetc_price = (result[8].room_lowcharge * (day-overday)) + (result[8].room_peakcharge * overday);
        room_result.site_grasstwo_price = (result[10].room_lowcharge * (day-overday)) + (result[10].room_peakcharge * overday);
      //}
      ...
      if ( overday >= 2) {
        ...
        room_result.scops_owl_price = (result[7].room_lowcharge * overday) + (result[7].room_peakcharge * (day-overday));
        room_result.site_cypress_price = (result[9].room_lowcharge * overday) + (result[9].room_peakcharge * (day-overday));
        room_result.site_zelkova_price = (result[11].room_lowcharge * overday) + (result[11].room_peakcharge * (day-overday));
        room_result.site_grass_price = (result[10].room_lowcharge * overday) + (result[10].room_peakcharge * (day-overday));
        room_result.firstetc_price = (result[2].room_lowcharge * overday) + (result[2].room_peakcharge * (day-overday));
        room_result.secondetc_price = (result[8].room_lowcharge * overday) + (result[8].room_peakcharge * (day-overday));
        room_result.site_grasstwo_price = (result[10].room_lowcharge * overday) + (result[10].room_peakcharge * (day-overday));
      //}
    } else {
      //errhandler 작성
    }
...
reserve.reserve=function(req,res){
  ...
  var data = [req.body.f_day+"&"+req.body.room_name, req.body.room_type, req.body.user_name, req.body.user_phone, req.body.f_day, req.body.l_day, userid, req.body.room_name]
  ...
  mysql.query_get(res, "select * from room_detail where room_type='"+req.body.room_type+"'", errhandler, function(result) {
    //console.log(result);
    if (parseInt(req.body.f_day[1])==6 && parseInt(req.body.l_day[1])==7) {
      overday=parseInt(req.body.l_day[2])-1;
      if ( ( parseInt(req.body.s_day) - overday) >= 2) {
        if(req.body.room_type=="site_cypress" || req.body.room_type=="site_zelkova" || req.body.room_type=="site_grass" || req.body.room_type=="raccoon" || req.body.room_type=="firstetc"){
            if(req.body.room_type=="site_grass"&&(req.body.addprice > 0)){
                    ...
                    price = result[0].room_lowcharge+((result[0].room_lowcharge - 0) * (parseInt(req.body.s_day)-overday-1)) + ((result[0].room_peakcharge) * overday);
            }else{
        ...
        price = (result[0].room_lowcharge * (parseInt(req.body.s_day)-overday)) + (result[0].room_peakcharge * overday);  
        }
        data.push(price);
      ...
      if ( overday >= 2) {
         if(req.body.room_type=="site_cypress" || req.body.room_type=="site_zelkova" || req.body.room_type=="site_grass" || req.body.room_type=="raccoon" || req.body.room_type=="firstetc"){
           if(req.body.room_type=="site_grass"&&(req.body.addprice > 0)){
              ...
              price = result[0].room_lowcharge+((result[0].room_lowcharge - 0) * (overday - 1)) + ((result[0].room_peakcharge) * (parseInt(req.body.s_day) - overday));
           }else{
             if(req.body.room_type=="site_grass"){
              price = result[0].room_lowcharge+((result[0].room_lowcharge - 0) * (overday - 1)) + ((result[0].room_peakcharge) * (parseInt(req.body.s_day) - overday));
             }else{
           ...
           price = (result[0].room_lowcharge * overday) + (result[0].room_peakcharge * (parseInt(req.body.s_day) - overday));
         }
        data.push(price);
      } else {
       price = (result[0].room_lowcharge * overday) + (result[0].room_peakcharge * (parseInt(req.body.s_day) - overday));
    ...
    for (var i=0; i<(req.body.s_day);i++) {
      data2.push([moment(new Date(req.body.f_day[0],req.body.f_day[1]-1,req.body.f_day[2])).add('days',i).format('YYYY[-]MM[-]DD')+"&"+req.body.room_name, req.body.room_type, req.body.room_name, moment(new Date(req.body.f_day[0],req.body.f_day[1]-1,req.body.f_day[2])).add('days',i).format('YYYY[-]MM[-]DD')])
    }
    ...
    mysql.data_put( res, sql, [data2], rev_handler, function(result) {
      ...
      if(result=="false")
      {
        ...
        sql = 'insert into room_reservation (rev_id, rev_roomtype, rev_name, rev_phone, rev_sday, rev_lday, user_id, room_name, rev_charge) values (?, ?, ?, ?, ?, ?, ?, ?, ?)'
...
reserve.timer=function(req, res) {
	if(req.session.logined!="yes"){
        mysql.reservegetdata(res, "select * from room_reservation where user_id=?", req.body.username+"&"+req.body.userphone, errhandler, function(rev_max) {
                mysql.reservegetdata(res, "select *, date_format(rev_cancel, '%Y-%m-%d %k:%i:%s') as timer, date_format(rev_sday, '%Y-%m-%d') as f_day,SUBSTRING_INDEX(room_reservation.rev_id, '&', -1) rev_id_split, date_format(rev_lday, '%Y-%m-%d') as l_day  from room_reservation where user_id=? limit ?, 10", [req.body.username+"&"+req.body.userphone, parseInt(req.body.limit)], errhandler, function(result) {

                        res.json({"timer":result, "max_rev":rev_max, "cnt":req.body.limit});
                });
});
	}else{
	mysql.reservegetdata(res, "select * from room_reservation where user_id=?", req.session.userid, errhandler, function(rev_max) {
		mysql.reservegetdata(res, "select *, date_format(rev_cancel, '%Y-%m-%d %k:%i:%s') as timer, date_format(rev_sday, '%Y-%m-%d') as f_day,SUBSTRING_INDEX(room_reservation.rev_id, '&', -1) rev_id_split, date_format(rev_lday, '%Y-%m-%d') as l_day  from room_reservation where user_id=? limit ?, 10", [req.session.userid, parseInt(req.body.limit)], errhandler, function(result) {
...
reserve.last_reservation=function(req, res) {
	mysql.reservegetdata(res, "select *, date_format(rev_sday, '%Y-%m-%d') as f_day, date_format(rev_lday, '%Y-%m-%d') as l_day from last_reservation where user_id=? limit ?, 10", [req.session.userid, parseInt(req.body.limit)], errhandler, function(result) {
...
reserve.day_query=function(req,res){
  var sql = "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',0).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',1).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',2).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',3).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',4).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',5).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',6).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',7).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',8).format('YYYY[-]MM[-]DD')+"')day group by room_type;"+
  "select room_type, count(*) AS room_cnt from (select * from rev_info  where rev_day='"+moment().add('days',9).format('YYYY[-]MM[-]DD')+"')day group by room_type;";
...
reserve.roomindex=function(req,res, test){
  ...
  var sql = 'select room_name from room_management where room_type=? and room_name NOT IN (select room_name from (select * from rev_info where rev_day >= date(?) and rev_day < date(?))date_query where room_type=?);';
