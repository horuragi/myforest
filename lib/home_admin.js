...
var admin_upload = multer({ storage: admin_storage,
...
var queryString = 'select idx,id,nickname,name,date_format(regdate, \'%Y-%m-%d\') as regdate from user order by idx desc limit '+no+','+page_size;
...
var queryString = 'select id,ip_address,referer,date_format(regdate, \'%Y-%m-%d\') as regdate from visit where regdate like "'+today+'%" order by id desc limit '+no+','+page_size;
