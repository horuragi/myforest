 ...
 */
const parkingAreaSelectInnerContainer = document.getElementsByClassName(
  "parkingAreaSelectInnerContainer"
)[0];
const horizontalCalenderContainer = document.getElementsByClassName(
  "horizontalCalenderContainer"
)[0];
      ...
      case "setPrevMonth":
        controller.setPrevMonth(({ year, month, dayLength, selectDate }) => {
          renderCalender({
            ...
            month,
            dayLength,
            selectedStartDate: selectDate.start,
            selectedEndDate: selectDate.end,
          });
        });
        break;
      case "setNextMonth":
...
function renderParkingArea(parkingAreas) {
  const pakingAreaSelectBoxs = parkingAreas
    .map(
      (area) =>
        `<div class="pakingAreaSelectBox${
          area.selected ? " areaClicked" : ""
        }" data-id="${area.id}">
...
function renderCalender({
  ...
  dates.innerHTML = Array.from({ length: dayLength })
    .map((_, i) => {
      const day = i + 1;
      return `<div class="${_getDateClassName(
        year,
        month,
        day
      )}" data-date="${year}-${
        month + 1
      }-${day}" data-action="selectDate">${day}</div>`;
    })
    .join("");
...
function renderSelectedInfo({
  ...
  selectedStartDate.innerHTML = selectDate?.start
    ? _dateFormat(selectDate.start)
    : "-";
  selectedEndDate.innerHTML = selectDate?.end
    ? _dateFormat(selectDate.end)
    : "-";
  sumOfParkingFee.innerHTML =
    selectDateLength && selectParkingArea?.price
      ? selectDateLength * selectParkingArea.price
      : "-";
...
function moveReservationPage() {
  ...
  if (!controller.isSelectParkingDate()) {
    alert("예약기간이 선택되지 않았습니다.");
    return;
  }

  const selectAreaText = selectedArea.innerText;
  const selectedStartDateText = selectedStartDate.innerText;
  const selectedEndDateText = selectedEndDate.innerText;
  const sumOfParkingFeeText = sumOfParkingFee.innerText;

  location.href = `/parking/reservation/area/${selectAreaText}/startDate/${selectedStartDateText}/endDate/${selectedEndDateText}/parkingFee/${sumOfParkingFeeText}`;
}
