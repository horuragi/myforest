...
var datas = "<%- data[8].picture %>";
if(datas==""){
        var room=$("<div id=\"link\"><div class=\"room\"><div class=\"room_img\"><img src=\"/images/noimage.gif\"></div><div class=\"room_details\"><p class=\"room_title\" >[방갈로] 꾀꼬리</p><br><span>전기온돌/냉장고/선풍기/에어컨/야외 테라스</span><br><br><span>예약 가능 좌석  <span >"+res.nightingale+"</span><p class=\"room_price\">"+res.nightingale_price+"</p></div></div></div>").appendTo(target);
}else{
var arr = JSON.parse(datas.picture); 
        var room=$("<div id=\"link\"><div class=\"room\"><div class=\"room_img\"><img src=\"/camp_image/"+arr[0].filename+"\"></div><div class=\"room_details\"><p class=\"room_title\" >[방갈로] 꾀꼬리</p><br><span>전기온돌/냉장고/선풍기/에어컨/야외 테라스</span><br><br><span>예약 가능 좌석  <span >"+res.nightingale+"</span><p class=\"room_price\">"+res.nightingale_price+"</p></div></div></div>").appendTo(target);
}        
    }

    if (res.bluebird > 0 ) {
        room=$("<div id=\"link2\"><div class=\"room\"><div class=\"room_img\"><img src=\"/images/bungalow/bluebird/1.jpg\"></div><div class=\"room_details\"><p class=\"room_title\" >[방갈로] 파랑새</p><br><span>전기온돌/냉장고/선풍기/에>어컨/야외 테라스</span><br><br><span>예약 가능 좌석  <span>"+res.bluebird+"</span>  <p class=\"room_price\">"+res.bluebird_price+"</p></div></div></div>").appendTo(target);
    }

    if (res.owl > 0 ){
        room=$("<div id=\"link3\"><div class=\"room\"><div class=\"room_img\"><img src=\"/images/bungalow/owl/1.jpg\"></div><div class=\"room_details\"><p class=\"room_title\" >[방갈로] 부엉이</p><br><span>전기온돌/냉장고/야외 테라스/야>외 파라솔</span><br><br><span>예약 가능 좌석  <span>"+res.owl+"</span> <p class=\"room_price\">"+res.owl_price+"</p></div></div></div>").appendTo(target);
    }
