  ...
  let handler = function (name) {
    for (var i = 0; i < data.result.length; i++) {
      ...
      if (index[1] == 'grass') {
        if (index.length == 3) {
          room_index = $(
            '<div class="check_info" style="height:53px;line-height:53px;font-size:0.8em"><div style="float:left;width:45%;height:50px;line-height:50px"><input type="checkbox" name="room_index" value="' +
              ...
              '_' +
              index[2] +
              '</label></div><div style="float:left;width:50%;height:50px;line-height:50px"><input type="checkbox" name="solo" onchange="toggleclick()" data-size="sm" data-toggle="toggle" class="custom-control-input"><label>두팀사용여부</labe></div></div>',
          ).appendTo(target);
        } else {
          room_index = $(
            '<div class="check_info" style="height:53px;line-height:53px;font-size:0.8em"><div style="float:left;width:45%;height:50px;line-height:50px"><input type="checkbox" name="room_index" value="' +
...
function room_name(name, handler) {
  ...
  if (room_type == '') {
    $.ajax({
      ...
      data: { room_type: name },
      success: function (response) {
        document.getElementsByClassName('getroomname')[0].innerHTML =
          '<strong>' + response.result[0].name + '</strong>';
