function terms(){
    ...
    if (sec_room.user_phone.value =="") {
        ...
        alert("방 번호를 선택해주세요.")
            ...
            var pricearr = sec_room.price_arr.value.split(',');
            for(var i=0;i<arr.length;i++){
                ...
                $.ajax({
                    ...
                    ,data: {room_type:sec_room.room_type.value,f_day:sec_room.f_day.value,l_day:sec_room.l_day.value,s_day:sec_room.s_day.value,room_name:arr[i],last_field:sec_room.last_field.value,user_name:sec_room.user_name.value,user_id:sec_room.user_id.value,user_phone:sec_room.user_phone.value,addprice:pricearr[i]}
...
function view_index(data) {
    ...
    let handler = function(name){
        for (var i=0; i<data.result.length; i++) {
	        ...
	        if(index[1]=="grass"){
	            if ( index.length == 3) {
		        room_index=$("<div class=\"check_info\" style=\"height:53px;line-height:53px;font-size:0.8em\"><div style=\"float:left;width:45%;height:50px;line-height:50px\"><input type=\"checkbox\" name=\"room_index\" value=\""+data.result[i].room_name+"\">"+"<label>"+name+"_"+index[2]+"</label></div><div style=\"float:left;width:50%;height:50px;line-height:50px\"><input type=\"checkbox\" name=\"solo\" onchange=\"toggleclick()\" data-size=\"sm\" data-toggle=\"toggle\" class=\"custom-control-input\"><label>두팀사용여부</labe></div></div>").appendTo(target);
	            } else {
		        room_index=$("<div class=\"check_info\" style=\"height:53px;line-height:53px;font-size:0.8em\"><div style=\"float:left;width:45%;height:50px;line-height:50px\"><input type=\"checkbox\" name=\"room_index\" value=\""+data.result[i].room_name+"\">"+"<label>"+name+"_"+index[1]+"</label>/div><div style=\"float:left;width:50%;height:50px;line-height:50px\"><input type=\"checkbox\" name=\"solo\" onchange=\"toggleclick()\" data-size=\"sm\" data-toggle=\"toggle\" class=\"custom-control-input\"><label>두팀사용여부</labe></div></div>").appendTo(target);
	            }
            }else{
                if ( index.length == 3) {
		        room_index=$("<div class=\"check_info\"><div class=\"ui checkbox\"><input type=\"checkbox\" name=\"room_index\" value=\""+data.result[i].room_name+"\">"+"<label>"+name+"_"+index[2]+"</label>").appendTo(target);
	            } else {
		        room_index=$("<div class=\"check_info\"><div class=\"ui checkbox\"><input type=\"checkbox\" name=\"room_index\" value=\""+data.result[i].room_name+"\">"+"<label>"+name+"_"+index[1]+"</label>").appendTo(target);
	            }
            }
        }
        ...
        $('input[type="checkbox"][name="room_index"]').click(function(){
            ...
            for(i=0;i<chkbox.length;i++) {
                if(chkbox[i].checked==true){
                    ...
                    var solo_check=chkbox[i].parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0];
                    console.log(solo_check)
                    if(solo_check.checked){
                        pricearr.push(sec_room.add_price.value*1*sec_room.s_day.value);
                    }else{
                        pricearr.push(0);
                    }
                }
            }
            sec_room.price_arr.value = pricearr;
            
            var price = document.getElementsByClassName('price_data')[0].innerHTML*1;
            var twoprice = document.getElementsByClassName('twoprice_data')[0].innerHTML*1;
            ...
            for(i=0;i<pricearr.length;i++){
                sum_add_price=sum_add_price+pricearr[i];
                if(pricearr[i]*1>0){
                    real_price = real_price+(twoprice*1);
                }else{
                    real_price = real_price+(price*1);
                }
            ...
            default_price.innerHTML=real_price*1+sum_add_price;
            sum_price.innerHTML=real_price*1+sum_add_price;
...
function room_name(name,handler) {
	switch(name){ 
        ...
        $.ajax({
            ...
            ,data: {room_type:name}
            ,success:function(response){
                document.getElementsByClassName('getroomname')[0].innerHTML = "<strong>"+response.result[0].name+"</strong>";
...
function addprice_query(){
    $.ajax({
        type: "POST"
        ,url: "/reserve/add_price"
        ,data: {room_type:sec_room.room_type.value}
        ,success:function(response){
            sec_room.add_price.value=response.result[0].lowcharge;
        }
        ,error:function(data){
            alert("error");
            history.back();
